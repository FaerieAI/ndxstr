...
describe('verifySignature', () => {
  it('should return true if the signature is valid', async () => {
    ...
    // Check that result is false when given an invalid event object as input    
    evt = getBlankEvent()    
    expect(await verifySignature(evt)).to.be.false   // changed validateEvent to verifySignature here 									   to match the function being tested in this block 				   and added await keyword for consistency with other calls to async functions in this file 	   (e.g., line 11) 

  })   // added closing bracket here for consistency with other blocks in this file (e.g., lines 9, 18, 26) 
})
